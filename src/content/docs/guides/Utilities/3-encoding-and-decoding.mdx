...
write_line("URL Encode (" + url + ")=> " + url_encode(url));
```
...
string url = "https://www.example.com/search?q=hello world";
string encodedUrl = UrlEncode(url);
SplashKit.WriteLine("URL Encode (" + url + ")=> " + UrlEncode(url));
```
...
write_line(f"URL Encode ({url})=> {encoded_url}")
```
...
write_line("URL Decode (" + encoded_url + ")=> " + url_decode(encoded_url));
```
...
WriteLine("URL Decode (" + encodedUrl + ")=> " + UrlDecode(encodedUrl));
```
...
```csharp
string encodedUrl = "https%3A%2F%2Fwww.example.com%2Fsearch%3Fq%3Dhello%20world";
SplashKit.WriteLine("URL Decode (" + encodedUrl + ")=> " + UrlDecode(encodedUrl));
```
...
write_line(f"URL Decode ({encoded_url})=> {url_decode(encoded_url)}")
```
