...
int shift = 3;
std::cout << "Encoded: " << encoded_message << std::endl;
std::cout << "Decoded: " << ceaser_cipher_decode(encoded_message, shift) << std::endl;
```
...
int shift = 3;
Console.WriteLine("Encoded: " + encodedMessage);
Console.WriteLine("Decoded: " + ceaserCipherDecode(encodedMessage, shift));
```
...
shift = 3
print(f"Encoded: {encoded_message}")
print(f"Decoded: {ceaser_cipher_decode(encoded_message, shift)}")
```
...
void ceaser_cipher_brute_force(const string &input)
{
    ...
    {
        std::cout << "Shift " << shift << "=> " << ceaser_cipher_decode(input, shift) << std::endl;
    }
}
```
...
void ceaserCipherBruteForce(string input)
{
    for (int shift = 0; shift < 26; shift++)
    {
        Console.WriteLine("Shift " + shift + "=> " + ceaserCipherDecode(input, shift));
    }
}
```
...
def ceaser_cipher_brute_force(input_string):
    for shift in range(26):
        print(f"Shift {shift}=> {ceaser_cipher_decode(input_string, shift)}")
```
