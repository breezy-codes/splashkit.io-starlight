...
This guide will show you how to extend the programs we built in the [previous guide](/guides/networking/3-connecting-programs) to include messaging capabilities. We will build a server program that listens for incoming connections and processes messages sent by clients. The server program will then send a confirmation message back to the client. We will also build a client program that connects to the server and sends messages. The client program will then receive a confirmation message from the server.

Similar to how the concepts in the previous guide could be extended in countless ways, you could use this tutorial as an opportunity to test your skills and extend this guide to build a 2 way messaging application.
