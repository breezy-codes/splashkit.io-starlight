...
In our server, as we are working with only one program connecting to it. We use the `last_connection` function, as detailed [here](/api/networking/#last-connection), for assigning the 
