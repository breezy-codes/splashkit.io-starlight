...
- [Connecting Programs](/guides/networking/3-connecting-programs)
- [Building a Messaging Application](/guides/networking/4-messaging-application)
...
To send messages, we can use a variety of different Splashkit functions to send, process, and receive messages. Some of these functions are essential for enabling message communication, such as the `has_messages` function, so be sure to look through these if you're thinking on building your own chat application.
...
In our messaging application tutorial, found [here](/guides/networking/4-messaging-application), we use the following to send a message to the server. This function sends the message to the server and prints a message to the terminal to confirm that the message has been sent.
...
A key part of being able to send messages over these programs is to have a loop that continually checks for messages. Similar to how we did for the `check_network_activity` in order to keep the connection open, here we do the same with the `has_messages_on_connection` function, as detailed [here](/api/networking/#has-messages-2). We do this same if loop in both the server and client, so when you're building your programs, think about how you can ensure you're getting the programs to check for the things you want to accomplish.
